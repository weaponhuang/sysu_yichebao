...
public class GetBikeListByLabelAsyncTask extends
	...
	public GetBikeListByLabelAsyncTask(String label, int startBikeId, int size,
		...
		if (result != null) {
			try {
				...
				if (requestCode == 1) {
					...
					listener.onGetBikeListByLabelSuccess(bikeList);
				} else {
					listener.onGetBikeListByLabelFail("获取列表失败！");
				}
			} catch (JSONException e) {
				e.printStackTrace();
				listener.onGetBikeListByLabelFail("返回错误！");
			}
		} else {
			listener.onGetBikeListByLabelFail("网络连接错误！");
		}
	}

	public interface OnRequestListener {
		void onGetBikeListByLabelSuccess(List<Map<String, Object>> bikeList);

		void onGetBikeListByLabelFail(String errorMessage);
	}
}
